...
def extractLines(img):
	...
	for i, sum in enumerate(hpList):
		...
		if(sum==0):
			...
			setSpaceTop = True # next time we encounter 0, it's begining of another
		# space so we set new spaceTop
		...
		if(sum>0):
			...
			if(i<len(hpList)-1): # this condition is necessary to avoid array index out of bound error
				if(hpList[i+1]>0): # if the next horizontal projectin is > 0, keep on counting,
				...
				if(lineBottom-lineTop<20):
					...
					continue
			includeNextSpace = True # the line/contour is accepted, new space following it will
			# be accepted
...
def extractSlant(img, words):
	...
	# convert to grayscale and binarize the image by INVERTED binary thresholding
	# it's better to clear unwanted dark areas at the document left edge and use a high threshold value
	# to preserve more text pixels
	thresh = threshold(filtered, 180)
	
	# loop for each value of angle in theta
	for i, angle in enumerate(theta):
		s_temp = 0.0 # overall sum of the functions of all the columns of all the words!
		count = 0 # just counting the number of columns considered to contain a vertical stroke and
		# thus contributing to s_temp
		...
		for j, word in enumerate(words):
			...
			# it's divided by 2 because the uppermost point and the lowermost points are being equally shifted
			...
			for k, sum in enumerate(vp):
				...
				# if number of foreground pixels is less than onethird of total pixels, it is not a
				# vertical stroke so we can ignore
				...
				# the distance is found as delta_y, I just followed the naming convention in the research paper
				# I followed
				delta_y = height - (l+m)
				...
				# this will also somewhat negate the effect of adding more columns and different column counts
				# in the transformed image of the same word
...
def main():
	...
	image = cv2.imread('images/m3.png')
