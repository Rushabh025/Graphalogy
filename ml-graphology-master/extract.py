...
''' function for finding contours and straightening them horizontally. Straightened lines 
will give better result with horizontal projections. '''


def straighten(image, contour_count=None):
    ...
    for i, ctr in enumerate(ctrs):
        ...
        # If the length of the line is less than one third of the document width, especially
        # for the last line,ignore because it may yeild inacurate baseline angle which subsequently
        # affects proceeding features.
...
def extractLines(img):
    ...
    # convert to grayscale and binarize the image by INVERTED binary thresholding
    # it's better to clear unwanted dark areas at the document left edge and use a high
    # threshold value to preserve more text pixels
    ...
    # FIRST we extract the straightened contours from the image by looking at occurance of
    # 0's in the horizontal projection.
    lineTop = 0
    lineBottom = 0
    spaceTop = 0
    spaceBottom = 0
    ...
    for i, sum in enumerate(hpList):
        ...
        if (sum == 0):
            ...
            if (i < len(hpList) - 1):  # this condition is necessary to avoid array index out of bound error
                if (hpList[
                    i + 1] == 0):  # if the next horizontal projectin is 0, keep on counting,
            ...
            setSpaceTop = True  # next time we encounter 0, it's begining of another space so we
            # set new spaceTop
        ...
        if (sum > 0):
            ...
            if (i < len(hpList) - 1):  # this condition is necessary to avoid array index out of bound error
                ...
                if (lineBottom - lineTop < 20):
                    ...
                    setLineTop = True  # next time we encounter value > 0, it's begining of another
                    # line/contour so we set new lineTop
                    continue
    ...
    for i, line in enumerate(lines):
        ...
        anchor = line[
            0]  # 'anchor' will locate the horizontal indices where horizontal projection
        # is > ANCHOR_POINT for uphill or < ANCHOR_POINT for downhill(ANCHOR_POINT is arbitrary yet suitable!)
        anchorPoints = []  # python list where the indices obtained by 'anchor' will be stored
        upHill = True  # it implies that we expect to find the start of an individual line
        # (vertically), climbing up the histogram
        downHill = False  # it implies that we expect to find the end of an individual line
        # (vertically), climbing down the histogram
        segment = hpList[
                  line[0]:line[1]]  # we put the region of interest of the horizontal projection
        ...
        for x in range(1, len(anchorPoints) - 1, 2):
            ...
            # this is a weakness of the algorithm to extract lines (anchor value is
            # ANCHOR_POINT, see for different values!)
            if (lineBottom - lineTop < 20):
                continue
    ...
    # For this, we set an arbitrary (yet suitable!) threshold MIDZONE_THRESHOLD = 15000 in horizontal
    # projection to identify the midzone containing rows.
    # These two total numbers will be divided by number of lines (having at least one
    # row>MIDZONE_THRESHOLD) to find average line spacing and average letter size.
    ...
    total_space_row_count = space_nonzero_row_count + np.sum(
        space_zero[1:-1])  # excluding first and last entries: Top and Bottom margins
    # the number of spaces is 1 less than number of lines but total_space_row_count contains
    # the top and bottom spaces of the line
    ...
    # We can't just take the average_line_spacing as a feature directly. We must take the
    # average_line_spacing relative to average_letter_size.
    # Let's take the ratio of average_line_spacing to average_letter_size as the LINE SPACING,
    # which is perspective to average_letter_size.
...
def extractWords(image, lines):
    ...
    # Isolated words or components will be extacted from each line by looking at occurance
    # of 0's in its vertical projection.
    for i, line in enumerate(lines):
        ...
        for j, sum in enumerate(vp):
            ...
            if (sum == 0):
                if (setSpaceStart):
                    ...
                    setSpaceStart = False  # spaceStart will be set once for each start of a space
                    # between lines
                indexCount += 1
                spaceEnd = indexCount
                if (j < len(vp) - 1):  # this condition is necessary to avoid array index out of bound error
                    if (vp[
                        j + 1] == 0):  # if the next vertical projectin is 0, keep on counting,
                        # it's still in blank space
                        continue
                ...
                setSpaceStart = True  # next time we encounter 0, it's begining of another space so
                # we set new spaceStart
            ...
            if (sum > 0):
                ...
                if (j < len(vp) - 1):  # this condition is necessary to avoid array index out of bound error
                    if (vp[
                        j + 1] > 0):  # if the next horizontal projectin is > 0, keep on counting,
                ...
                setWordStart = True  # next time we encounter value > 0, it's begining of another
                # word/component so we set new wordStart
...
def extractSlant(img, words):
    global SLANT_ANGLE
    '''
	0.01 radian = 0.5729578 degree :: I had to put this instead of 0.0 becuase there was a bug 
	yeilding inacurate value which I could not figure out!
    ...
    # theta = [-0.785398, -0.523599, -0.436332, -0.349066, -0.261799, -0.174533, -0.0872665, 0, 0.0872665,
    # 0.174533, 0.261799, 0.349066, 0.436332, 0.523599, 0.785398]

    # Corresponding index of the biggest value in s_function will be the index of the most likely
    # angle in 'theta'
    s_function = [0.0] * 9
    count_ = [0] * 9
    ...
    # convert to grayscale and binarize the image by INVERTED binary thresholding
    # it's better to clear unwanted dark areas at the document left edge and use a high threshold
    # value to preserve more text pixels
    thresh = threshold(filtered, 180)
    # cv2.imshow('thresh', lthresh)
    ...
    for i, angle in enumerate(theta):
        ...
        count = 0  # just counting the number of columns considered to contain a vertical
        # stroke and thus contributing to s_temp
        ...
        for j, word in enumerate(words):
            ...
            # it's divided by 2 because the uppermost point and the lowermost points are being equally
